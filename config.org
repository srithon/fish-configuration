...
function resume
    ...
    set path $git_root/exports/SridaranThoniyil.pdf

    if status is-command-substitution
        echo $path
    else if test (count $argv) -gt 0
        # then, invoke resume searcher
        perl ~/.config/fish/utils/resume-searching/search_resume.pl $git_root $argv
    else
        spawn okular $path
    end
end
...
my $resumeFilename = basename(abs_path("$gitPath/exports/SridaranThoniyil.pdf"));
my $pattern = qr/_(.+)\.pdf/;
...
open my $tex, "git show $commit:jakesResume.tex |";
