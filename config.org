...
function __remote-desktop-completion -d "Yields (file, description) pairs for `remote-desktop` function"
    # get a list of files, and for each file extract the ~Name=~ line
    set PROFILE_DIR ~/.local/share/remmina

    set PROFILES (fd --base-directory $PROFILE_DIR . -tfile)
    set PROFILE_NAMES (rg --no-filename --only-matching --replace '$1' '^name=(.+)' $PROFILES)

    for i in (seq 1 (count $PROFILES))
        echo -e "$PROFILES[$i]\t$PROFILE_NAMES[$i]"
    end
end
...
# next, let's parse the ZFS output
my @zfsMountpoints = ();
open FD, "zfs list -H -o mountpoint |";
while (<FD>) {
    chomp;
    # skip datasets with no mountpoint
    next if ($_ =~ /^-$/);
    push @zfsMountpoints, $_;
}

close FD;

# now, let's make a regex which will match the longest prefix.
# this regex is made up of alternatives, where earlier items have longer lengths
# than later ones.
my $regex = join "|", sort { length $b <=> length $a } @zfsMountpoints;

# make the regex match only at the beginning.
# the second capture group matches the path relative to the volume.
$regex = "(^$regex)(.*)";

# now, let's see which one matches
if ($targetPath =~ $regex) {
...
Let's make an alias to make it easier to use.
#+begin_src fish
